...
void ColoursPalette::onGlow(CCObject *sender)
{
    CCMenuItemSpriteExtra *obj = (CCMenuItemSpriteExtra *)sender;
    this->glow = !this->glow;
...
void ColoursPalette::addColorButton(CCMenu *father, float posX, float posY, int id)
{
    auto sprite = cocos2d::CCSprite::createWithSpriteFrameName("GJ_colorBtn_001.png");
...
bool ColoursPalette::init(GJGarageLayer *garageLayer)
{

    if (this->initWithColor({0x00, 0x00, 0x00, 0x3B}))
    {
        this->garageLayer = garageLayer;
        ...
        auto icons_bg = cocos2d::extension::CCScale9Sprite::create("square02_001.png");
        icons_bg->setContentSize(cocos2d::CCSize(415.0f, 55.0f));
        icons_bg->setOpacity(50);
        icons_bg->setPosition(win_size.width / 2, win_size.height / 2 + 90);
...
void ColoursPalette::do_hooks(void *lib)
{
    extern void *getPointerFromSymbol(void *handle, const char *symbol);

    // HookManager::do_hook(getPointerFromSymbol(lib,"_ZN11GameManager11colorForIdxEi"), getPointer(&ColorsPalette_hooks::GameManager_colorForIdx_hook),(void **)&ColorsPalette_hooks::GameManager_colorForIdx_trp);
}